...
namespace BugTracker.Controllers
{
    public class TicketAttachmentsController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,TicketId,FilePath,Description,Created,UserId,FileUrl")] TicketAttachment ticketAttachments, HttpPostedFileBase image)
        {
            if (ModelState.IsValid)
            {
                ...
                if (ImageUploadValidator.IsWebFriendlyImage(image))
                {
                    var fileName = Path.GetFileName(image.FileName);
                    image.SaveAs(Path.Combine(Server.MapPath("~/img/attachments/"), fileName));
                    ticketAttachments.FileUrl = "~/img/attachments/" + fileName;
                }
                db.SaveChanges();
